Shader "Custom/FakeCausticsDistortion" {
	Properties {
		...
		_DTexture("Distort (RGB)", 2D) = "white" {}
		_Distort("_DistortAmount", Range(0,1)) = .2
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutput o) {
			...
			//pan.x = pan.x + _Time.r;
			o.Albedo = tex2D(_MainTex, IN.uv_MainTex + _Distort*tex2D(_DTexture, IN.uv_CTexture - _Time.r * _PanSpeed))*_Color + tex2D(_CTexture, pan)*IN.color.rgb*_CausticColor;
			o.Alpha = 1;
		}
		ENDCG
	} 
